/**
 * ERC-8004 Auto-Registration for Caliguland
...
export class ERC8004RegistryClient {
  ...
  async initialize(): Promise<void> {
    // Load IdentityRegistry contract
    const possiblePaths = [
    ...
    if (!contractAddress || !abi) {
      throw new Error('IdentityRegistry contract not found. Deploy contracts first: cd contracts && forge script script/DeployLiquiditySystem.s.sol --rpc-url http://localhost:8545 --broadcast');
    }
  ...
  async register(serverUrl: string, gameName: string): Promise<RegistrationResult> {
    ...
    await this.setMetadata(agentId, 'gameType', 'prediction-market');
...
async function detectAndDeployContracts(): Promise<string | null> {
  ...
  for (const path of possiblePaths) {
    if (existsSync(path)) {
      ...
      if (deployment.identityRegistry) {
        console.log(`[ERC-8004] Found deployed contracts`);
  ...
  if (chainId === 31337) {
    ...
    if (!existsSync(contractsDir)) {
      console.log('[ERC-8004] Contracts directory not found');
      return null;
    }
    ...
    if (deployProc.exitCode === 0) {
      console.log('[ERC-8004] ✅ Contracts auto-deployed');
